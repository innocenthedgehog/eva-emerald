...
// Move data settings
#define B_UPDATED_MOVE_DATA         GEN_CUSTOM // Updates move data in gMovesInfo, including Power, Accuracy, PP, category, range, and additional effects.
#define B_UPDATED_MOVE_TYPES        GEN_CUSTOM // Updates move types in gMovesInfo.
#define B_UPDATED_MOVE_FLAGS        GEN_CUSTOM // Updates move flags in gMovesInfo.
#define B_PHYSICAL_SPECIAL_SPLIT    GEN_CUSTOM // In Gens1-3, a move's type determines if it will do physical or special damage. The category icon in the summary will reflect this.
...
#define B_UPDATED_ABILITY_DATA      GEN_CUSTOM // Affects flags.
...
#define B_RETURN_STOLEN_NPC_ITEMS   GEN_3 // In Gen5+, Thief and Covet no longer permanently steal items from NPCs.
...
#define B_SAFARI_BALL_MODIFIER      GEN_3 // In Gen8+, Safari Ball's catch multiplier is 1x. In Gens2-6, it's 1.5x. In Gen1, it's 2x.
...
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_AI_VS_AI_BATTLE     // If this flag is set, the player's Pokémon will be controlled by the AI in the next battle.
...
#define B_NUM_LOW_HEALTH_BEEPS          NUM_BEEPS_GEN_3 // This controls the number of times the "low HP" beep will loop. Setting this value to NUM_BEEPS_OFF will disable the beep, while NUM_BEEPS_GEN_3 will loop infinitely. You can set this to any number you want, the defines listed are just for ease of use.
